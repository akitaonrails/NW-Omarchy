...
- **Workspace-switch animation.** Hyprland's `workspace_swipe` pans the new workspace in under your finger continuously through the gesture. We tried building a coherent pan on picom v13 (custom `offset-x` keyed off `window-monitor-width`) and reverted — direction wasn't reversible (picom doesn't know next vs prev) and the screenshot overlay got animated through the same trigger. Shipping config does plain opacity fade on workspace switch. See [`gaps.md`](gaps.md).
